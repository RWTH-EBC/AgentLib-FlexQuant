...
# load the results
BASE_SIM = 'mpc_sim_simple_building_base.csv'
POS_SIM = 'mpc_sim_simple_building_pos_flex.csv'
NEG_SIM = 'mpc_sim_simple_building_neg_flex.csv'
W_SIM_KEY = [BASE_SIM, POS_SIM, NEG_SIM]
INTERP_W_SIM_KEY = ['interp_' + key for key in W_SIM_KEY]

BASE_COLL = 'mpc_simple_building_base.csv'
POS_COLL = 'mpc_simple_building_pos_flex.csv'
NEG_COLL = 'mpc_simple_building_neg_flex.csv'
...
plot_all('P_el', lb=-50, ub=250, ylabel="$P_{el}$ / kW", dt=dt, interpolation=False)
# plot_all('T_out', lb=290, ub=300, ylabel="$T_{out}$ / K", dt=dt, interpolation=False)

time_step_for_plot = 900
plot_one_step(variable='P_el', lb=-50, ub=250, ylabel="$P_{el}$ / kW", time_step=time_step_for_plot, dt=dt, interpolation=False)
