...
def get_series_from_predictions(series, convert_to="seconds", fname=None, return_first=False, index_of_return=0, handle_col_vals=False):
...
def plot_power_simulation(results, fmu=False, detailed=False, size=(6.4,4.8), convert_to="seconds", tikz=False):
    if detailed:
        ...
        fig, ax = plt.subplots(figsize=size)
    if fmu:
        arr = 1000 * (results["SimAgent"]["SimTestHall"]["Q_Ahu"] + results["SimAgent"]["SimTestHall"]["Q_Tabs"]) / get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['parameter']['COP']).iloc[0]
    else:
    ...
    for time, flex_res in results["FlexibilityMarket"]["FlexibilityMarket"].groupby(level=0):
        missing_indices = results["NegFlexMPC"]['NegFlexMPC']['variable']['P_el_c'][time][np.isnan(results["NegFlexMPC"]['NegFlexMPC']['variable']['P_el_c'][time])].index
        time_step = missing_indices[1] - missing_indices[0]
    
        if flex_res["Status"].iloc[0] == OfferStatus.accepted_positive.value:
            profile = flex_res["Base Profile"][time] - flex_res["Positive Profile"][time] * flex_res["Power Multiplier"][time].iloc[0]
        elif flex_res["Status"].iloc[0] == OfferStatus.accepted_negative.value:
            profile = flex_res["Base Profile"][time] + flex_res["Negative Profile"][time] * flex_res["Power Multiplier"][time].iloc[0]
    ...
    if detailed:
        preds = [get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['variable']['P_el_c'], handle_col_vals=True, return_first=True, index_of_return=i) for i in range(0, 24, 6)]
        for pred in preds:
            pred.index = pred.index / _CONVERSION_MAP[convert_to]
            axes[0].plot(pred)
        ...
        axes[1].plot(get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['variable']['Q_Tabs_set_del'], convert_to=convert_to,handle_col_vals=True), color="red",drawstyle="steps-post")
        axes[1].set_ylabel("Q_Tabs")
        preds = [get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['variable']['Q_Tabs_set_del'], handle_col_vals=True, return_first=True, index_of_return=i) for i in range(0, 24, 6)]
        for pred in preds:
            pred.index = pred.index / _CONVERSION_MAP[convert_to]
            axes[1].plot(pred)
        ...
        axes[2].plot(get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['variable']['Q_Ahu'], convert_to=convert_to,handle_col_vals=True), color="red",drawstyle="steps-post")
        axes[2].set_ylabel("Q_Ahu")
        preds = [get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['variable']['Q_Ahu'],handle_col_vals=True, return_first=True, index_of_return=i) for i in range(0, 15)]
        for pred in preds:
            pred.index = pred.index / _CONVERSION_MAP[convert_to]
            axes[2].plot(pred)

        axes[3].plot(get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['parameter']['r_pel'], convert_to=convert_to), color="black",drawstyle="steps-post")
...
def plot_prediction(results, times=[], size=(6.4,4.8), detailed=False, tikz=False):
    ...
    for time in times:
        ...
        a[0].plot(results["myMPCAgent"]['FlexMPC']['variable']['P_el_c'][time].dropna(), color="black",drawstyle="steps-post")
        a[0].plot(results["PosFlexMPC"]['PosFlexMPC']['variable']['P_el_c'][time].dropna(), color="red",drawstyle="steps-post")
        a[0].plot(results["NegFlexMPC"]['NegFlexMPC']['variable']['P_el_c'][time].dropna(), color="blue",drawstyle="steps-post")
...
### set xticklabels
### fix axis names
def plot_flex_price_graph(result_wo_flex, flex_results_list, flex_call_time, electricity_prices, size=(6.4,4.8), tikz=False, fmu=False):
    ...
    for res in flex_results_list:
        if not fmu:
            ...
            p_el_with = 1000 * (res["SimAgent"]["SimTestHall"]["Q_Ahu"] + res["SimAgent"]["SimTestHall"]["Q_Tabs"]) / get_series_from_predictions(res["myMPCAgent"]['FlexMPC']['parameter']['COP']).iloc[0]
        ...
        if fmu:
            p_el_wo = 1000 * (result_wo_flex["SimAgent"]["SimTestHall"]["Q_Ahu"] + result_wo_flex["SimAgent"]["SimTestHall"]["Q_Tabs"]) / get_series_from_predictions(result_wo_flex["myMPCAgent"]['FlexMPC']['parameter']['COP']).iloc[0]
        else:
    ...
    for time, offer in results['FlexibilityMarket']['FlexibilityMarket'].groupby(level=0):
        ...
        try:
            ...
            if not fmu:
                ...
                profile = (results["SimAgent"]["SimTestHall"]["Q_Ahu"] + results["SimAgent"]["SimTestHall"]["Q_Tabs"]) / get_series_from_predictions(results["myMPCAgent"]['FlexMPC']['parameter']['COP']).iloc[0]
            ...
            continue
    positive_flex = get_series_from_predictions(results["FlexibilityIndicator"]["FlexibilityIndicator"]["energyflex_pos"], convert_to=convert_to)
    negative_flex = - get_series_from_predictions(results["FlexibilityIndicator"]["FlexibilityIndicator"]["energyflex_neg"], convert_to=convert_to)
    if not tikz:        
        for time, val in data.items():
            ax.bar(time+2000/ _CONVERSION_MAP[convert_to], val[0], width=4000/_CONVERSION_MAP[convert_to], color="blue")
            ax.bar(time-2000/ _CONVERSION_MAP[convert_to], val[1], width=4000/_CONVERSION_MAP[convert_to], color="red")

        ax.plot(positive_flex, color="rosybrown", linewidth=0.5)
        ax.plot(negative_flex, color="cornflowerblue", linewidth=0.5)
        ax.legend(["Energieflexibilität Positiv", "Energieflexibilität Negative", "Angebotsmenge", "Abgerufene Flexibilität"])
        ax.set_xlabel('Zeit')
