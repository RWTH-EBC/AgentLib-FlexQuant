...
def run_example(until=until):
    """ Run the SimpleBuilding MAS example with a custom flexibility plugin.

    The custom plugin is implemented in agentlib_plugin, loaded via the
    flexibility_agent_config, and configured in market.json.

    See the README for a detailed step-by-step guide. As this example is based on the SimpleBuilding
    example, further details can be found inside its example folder. 
    """
