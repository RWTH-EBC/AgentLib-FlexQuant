...
from agentlib.core.agent import AgentConfig, Agent
from agentlib.core.errors import ConfigurationError
from flexibility_quantification.data_structures.flexquant_datatypes import FlexQuantConfig, \
...
from flexibility_quantification.data_structures.mpc_setup import BaseMPCData, BaselineMPCData, PFMPCData
from flexibility_quantification.utils.config_management import get_module, to_dict_and_remove_unnecessary_fields
...
class FlexAgentGenerator:
    ...
    def __init__(self, flex_config: Union[str, FilePath, FlexQuantConfig],
        ...
        self.neg_flex_mpc_agent_config = load_config.load_config(mpc_agent_config,
                                                                 config_type=AgentConfig)
        # baseline module
        self.baseline_mpc_module_config = get_module(config=self.baseline_mpc_agent_config,
                                                     module_type=mmap.MPC_CONFIG_TYPE)
        # pos module
        self.pos_flex_mpc_module_config = get_module(config=self.pos_flex_mpc_agent_config,
                                                     module_type=mmap.MPC_CONFIG_TYPE)
        # neg module
        self.neg_flex_mpc_module_config = get_module(config=self.neg_flex_mpc_agent_config,
                                                     module_type=mmap.MPC_CONFIG_TYPE)
        ...
        self.indicator_agent_config = load_config.load_config(self.flex_config.indicator_config.agent_config,
                                                              config_type=AgentConfig)
        self.indicator_module_config = get_module(config=self.indicator_agent_config,
                                                  module_type=mmap.INDICATOR_CONFIG_TYPE)
        # load market config
    ...
    def generate_flex_agents(self) -> [MPCConfig, MPCConfig, MPCConfig,
        ...
        baseline_mpc_config = self.adapt_mpc_module_config(module_config=self.baseline_mpc_module_config,
        ...
        self.append_module_and_dump_agent(module=baseline_mpc_config, agent=self.baseline_mpc_agent_config,
                                          module_type=mmap.MPC_CONFIG_TYPE,
                                          config_name=self.flex_config.baseline_config_generator_data.name_of_created_file)
        self.append_module_and_dump_agent(module=pf_mpc_config, agent=self.pos_flex_mpc_agent_config,
                                          module_type=mmap.MPC_CONFIG_TYPE,
                                          config_name=self.flex_config.shadow_mpc_config_generator_data.pos_flex.name_of_created_file)
        self.append_module_and_dump_agent(module=nf_mpc_config, agent=self.neg_flex_mpc_agent_config,
                                          module_type=mmap.MPC_CONFIG_TYPE,
                                          config_name=self.flex_config.shadow_mpc_config_generator_data.neg_flex.name_of_created_file)

        # same for indicator and market
        indicator_module_config = self.adapt_indicator_config(module_config=self.indicator_module_config)
        self.append_module_and_dump_agent(module=indicator_module_config, agent=self.indicator_agent_config,
                                          module_type=mmap.INDICATOR_CONFIG_TYPE,
        ...
        self._generate_flex_model_definition(
            ...
            weights=[weight.dict() for weight in self.flex_config.shadow_mpc_config_generator_data.weights],
    ...
    def append_module_and_dump_agent(self, module: BaseModuleConfig, agent: AgentConfig,
        ...
        module_dict = to_dict_and_remove_unnecessary_fields(module=module)
        # write given module to agent config
        for i, agent_module in enumerate(agent.modules):
            if mmap.MODULE_TYPES[module_type] is mmap.MODULE_TYPES[agent_module["type"]]:
    ...
    def adapt_mpc_module_config(self, module_config: MPCConfig, mpc_dataclass: BaseMPCData) -> MPCConfig:
        ...
        module_config.optimization_backend["model"]["type"] = {"file": os.path.join(self.flex_config.path_to_flex_files,
        ...
        if type(mpc_dataclass) is not BaselineMPCData:
            for control in module_config.controls:
                module_config.inputs.append(MPCVariable(name=f"_{control.name}", value=control.value))
        else:
            for control in module_config.controls:
                module_config.outputs.append(MPCVariable(name=control.name + mpc_dataclass.full_trajectory_suffix, value=control.value))
        ...
        if self.flex_config.baseline_config_generator_data.power_variable in output_dict:
            output_dict[self.flex_config.baseline_config_generator_data.power_variable].alias = mpc_dataclass.power_alias
        else:
            module_config.outputs.append(MPCVariable(name=self.flex_config.baseline_config_generator_data.power_variable, alias=mpc_dataclass.power_alias))
        # add inputs for the Time variable as well as extra inputs needed for activation of flex
        module_config.inputs.append(MPCVariable(name="Time", value=[i*module_config.time_step for i in range(module_config.prediction_horizon)]))
    ...
    def adapt_market_config(self, module_config: FlexibilityMarketModuleConfig) \
        ...
        """
        for parameter in module_config.parameters:
            if parameter.name in self.market_config.__fields__.keys():
                parameter.value = getattr(self.market_config, parameter.name)
            if parameter.name == "time_step":
                parameter.value = self.baseline_mpc_module_config.time_step
        return module_config
    ...
    def _generate_flex_model_definition(self, created_file_name, casadi_model_data, controls,
        ...
        if self.flex_config.overwrite_files:
            try:
                Path(os.path.join(self.flex_config.path_to_flex_files, self.flex_config.baseline_config_generator_data.created_flex_mpcs_file)).unlink()
            except OSError:
                pass
