...
class FlexibilityIndicatorModule(agentlib.BaseModule):
    ...
    def check_power_end_deviation(self, tol: float):
        ...
        if abs(dev_pos) > tol:
            logger.warning(f"There is an average deviation of {dev_pos:.6f} kW between the final values of power profiles of positive shadow MPC and the baseline. Correction of energy costs may be necessary.")
        if abs(dev_neg) > tol:
            logger.warning(f"There is an average deviation of {dev_pos:.6f} kW between the final values of power profiles of negative shadow MPC and the baseline. Correction of energy costs may be necessary.")
