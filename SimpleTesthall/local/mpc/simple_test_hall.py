...
class SimpleTestHallModelConfig(CasadiModelConfig):
    inputs: List[CasadiInput] = [
        # controls
        CasadiInput(name="T_flow_in", value=328.15, unit="K", description="flow temperature of radiator"),
        # disturbances
    ...
    outputs: List[CasadiOutput] = [
        CasadiOutput(name="P_el_alg", unit='W', description="Output for controll variable"),
        CasadiOutput(name="T_flow_out_baseline", unit="K", description="Temperature of zone"),
        CasadiOutput(name="Q_conv_debug", unit="W", description="Temperature of zone"),
        CasadiOutput(name="Q_rad_debug", unit="W", description="Temperature of zone"),
        #CasadiOutput(name="Q_flow_debug", unit='W', description="Output for controll variable"),
        CasadiOutput(name="Q_flow_total", unit='W', description="Output for controll variable"),
        CasadiOutput(name="T_int_sur_debug", unit='K', description="Output for controll variable"),
        CasadiOutput(name="T_ext_sur_debug", unit='K', description="Output for controll variable"),
        CasadiOutput(name="T_win_sur_debug", unit='K', description="Output for controll variable"),
        CasadiOutput(name="T_roof_sur_debug", unit='K', description="Output for controll variable"),
        #CasadiOutput(name="T_rad_debug", unit='K', description="radiative temperature"),
        CasadiOutput(name="T_rad", unit='K', description="radiative temperature")
    ]
...
class SimpleTestHall(CasadiModel):
    ...
    def setup_system(self):
        ...
        if has_floor:
            ...
            self.T_rad.alg = (coeff_dict['T_rad']['T_Air'] * self.T_Air +
        ...
        q_radiator_rad_1 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_1 ** 4 - self.T_rad ** 4)
        ...
        q_radiator_rad_2 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_2 ** 4 - self.T_rad ** 4)
        ...
        q_radiator_rad_3 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_3 ** 4 - self.T_rad ** 4)
        ...
        Q_flow_total = self.m_flow * self.cp_water * (self.T_flow_in - T_flow_out[layer - 1])
        #Q_flow_total = Q_flow_1 + Q_flow_2 + Q_flow_3
        self.Q_flow_total.alg = Q_flow_total
        self.P_el_alg.alg = ca.fabs(Q_flow_total) / self.COP

        #debug
        self.T_int_sur_debug.alg = self.T_IntWall_sur
        self.T_ext_sur_debug.alg = self.T_ExtWall_sur
        self.T_win_sur_debug.alg = self.T_Win_sur
        self.T_roof_sur_debug.alg = self.T_Roof_sur
        self.T_flow_out_baseline.alg = T_flow_out[layer - 1]
        self.Q_conv_debug.alg = q_radiator_conv_total
        self.Q_rad_debug.alg = q_radiator_rad_total
        self.T_preWin = self.T_preTemWin
        self.T_preWall = self.T_preTemWall
        self.T_preRoof = self.T_preTemRoof
        ...
        self.constraints = [
            ...
            (0, self.Q_flow_total + self.Q_flow_slack1 - self.Q_flow_slack2, 0),
            #(273.15, T_flow_out[layer - 1], self.T_flow_in)
        ]
        ...
        objective = sum(
            [
                ...
                (self.r_pel / 4) * (self.s_Pel / (self.s_T + self.s_Pel)) * (
                        self.Q_flow_slack1 + self.Q_flow_slack2) / self.COP / 1000,
            ]
        )
