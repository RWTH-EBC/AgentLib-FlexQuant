...
class SimpleTestHallModelConfig(CasadiModelConfig):
    inputs: List[CasadiInput] = [
        # controls
        CasadiInput(name="T_flow_in_min", value=328.15, unit="K", description="flow temperature of radiator"),
        # disturbances
    ...
    outputs: List[CasadiOutput] = [
        CasadiOutput(name="P_el_min_alg", unit='W', description="Output for controll variable"),
       CasadiOutput(name="T_rad_pf", unit='K', description="radiative temperature"),
        CasadiOutput(name="Q_flow_total_min", unit='W', description="Output for controll variable"),
        CasadiOutput(name="T_flow_out_PF", unit='K', description="Output temperature radiator")
    ]
...
class SimpleTestHall(CasadiModel):
    ...
    def setup_system(self):
        ...
        #####layer 1
        T_flow_in_1 = self.T_flow_in_min
        ...
        if has_floor:
            ...
            self.T_rad_pf.alg = (coeff_dict['T_rad']['T_Air'] * self.T_Air +
        ...
        q_radiator_rad_1 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_1 ** 4 - self.T_rad_pf ** 4)
        ...
        q_radiator_rad_2 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_2 ** 4 - self.T_rad_pf ** 4)
        ...
        q_radiator_rad_3 = self.sigma * eps_rad_twostar * A_in_internal * (
                T_radiator_sur_3 ** 4 - self.T_rad_pf ** 4)
        ...
        Q_flow_total = self.m_flow * self.cp_water * (self.T_flow_in_min - T_flow_out[layer - 1])
        #Q_flow_total = Q_flow_1 + Q_flow_2 + Q_flow_3
        self.Q_flow_total_min.alg = Q_flow_total
        self.P_el_min_alg.alg = ca.fabs(Q_flow_total) / self.COP

        # debug
        self.T_flow_out_PF.alg = T_flow_out[layer - 1]
        ...
        self.constraints = [
            ...
            (0, self.Q_flow_total_min + self.Q_flow_slack1 - self.Q_flow_slack2, 0),
            #(273.15, T_flow_out[layer - 1], self.T_flow_in_min)
        ]
